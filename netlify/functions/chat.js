...
<style>
:root{ --vh:1vh; --headerH:64px; --footerH:160px; --mobileNudge:-140px; }
@media (min-width:1025px){ #mn-chat{ position:relative; top:50vh; transform:translateY(-80%); max-width:720px; margin:0 auto; } }
@media (max-width:1024px){ #mn-chat{ max-width:640px; } }
@media (max-width:767px){
  html,body{ height:calc(var(--vh)*100)!important; overflow:hidden!important; margin:0; padding:0; }
  .mn-viewport{ height:calc(var(--vh)*100 - var(--headerH) - var(--footerH))!important; min-height:calc(var(--vh)*100 - var(--headerH) - var(--footerH))!important; display:flex!important; align-items:center!important; justify-content:center!important; margin:0!important; padding:0!important; }
  #mn-chat{ position:static!important; transform:none!important; max-width:94vw; margin:var(--mobileNudge) auto 0 auto!important; }
  .chaty-widget, .chaty-widget-container, [data-chaty]{ right:12px!important; bottom:96px!important; z-index:30!important; }
}
#mn-chat{
  --bg:#0B1516; --panel:#0E191A; --ink:#E6E8E6; --muted:#9FB7B3; --chip:#142224; --line:rgba(230,232,230,.10); --aqua:#00E6E6; --aqua2:#12B9B9;
  font-family:Montserrat,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif; color:var(--ink);
  max-width:680px; margin:20px auto; border-radius:16px; background:linear-gradient(180deg,var(--panel),var(--bg));
  border:1px solid rgba(230,232,230,.06); box-shadow:0 8px 24px rgba(0,0,0,.32);
}
#mn-chat .mn-title{ margin:14px 14px 4px; font-weight:700; font-size:18px; }
#mn-chat .mn-controls{ display:grid; grid-template-columns:1fr 1fr 1fr auto; gap:8px; padding:0 14px 10px; }
#mn-chat select, #mn-chat input[type="text"]{ height:36px; border-radius:10px; border:1px solid var(--line); background:#0F2426; color:var(--ink); padding:0 10px; font-size:14px; }
#mn-chat .mn-btn{ height:36px; padding:0 12px; border-radius:10px; border:1px solid rgba(0,230,230,.25); background:linear-gradient(180deg,var(--aqua2),var(--aqua)); color:#0F2426; font-weight:600; cursor:pointer; }
#mn-chat .mn-btn:active{ transform:translateY(1px); }
#mn-chat .mn-thread{ padding:6px 14px 8px; min-height:150px; max-height:52vh; overflow-y:auto; -webkit-overflow-scrolling:touch; }
...
#mn-chat .mn-composer{ display:flex; gap:8px; padding:6px 14px 14px; border-top:1px solid rgba(230,232,230,.06); }
#mn-chat .mn-composer input{ flex:1; background:#0F2426; color:var(--ink); caret-color:var(--ink); border:1px solid var(--line); border-radius:10px; height:40px; padding:0 11px; }
#mn-chat .mn-composer ::placeholder{ color:var(--muted); opacity:1; }
@media (max-width:1024px){ #mn-chat{ max-width:620px; } #mn-chat .mn-controls{ grid-template-columns:1fr 1fr; grid-auto-rows:36px; } }
@media (max-width:767px){ #mn-chat{ max-width:94vw; margin:12px auto; } #mn-chat .mn-controls{ grid-template-columns:1fr; } }
#mn-chat .sr-only{ position:absolute; left:-9999px; width:1px; height:1px; overflow:hidden; }
</style>

<script>
(function(){
  // keep CSS in sync with real viewport on mobile
  function setVars(){
...
<!-- =============== CHAT MARKUP =============== -->
<div id="mn-chat" class="ns-chat-wrap" data-endpoint="https://northstar-netlify.netlify.app/.netlify/functions/chat">
...
(function(){
  ...
  // Exact function URL (kept both as data attribute and fallback)
  const FALLBACK_ENDPOINT="https://northstar-netlify.netlify.app/.netlify/functions/chat";
